...
FLAGS="--verbose"
FLAGS=""
TIMEFORMAT="--format=TIME %Uuser %Ssystem %Eelapsed %PCPU (%Xtext+%Ddata %Mmax)k %Iinputs+%Ooutputs (%Fmajor+%Rminor)pagefaults %Wswaps"
...
function doit() {
  ...
  /usr/bin/time "$TIMEFORMAT" hexlite $FLAGS --flpcheck=none \
...
grep -H ^TIME *.log >time-summary.log
